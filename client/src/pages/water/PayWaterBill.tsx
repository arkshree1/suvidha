...
export default function PayWaterBill() {
  const { t } = useLanguage();
  const [step, setStep] = useState<'input' | 'details' | 'payment' | 'processing' | 'receipt'>('input');
  const [accountNo, setAccountNo] = useState('PCB98765');
  const [paymentMethod, setPaymentMethod] = useState('');
  const bill = mockCitizen.accounts.water;
  ...
  if (step === 'receipt') return (
    <div className="animate-fadeIn">
      <Breadcrumb items={[{ label: t('water'), path: '/water' }, { label: t('payWaterBill') }]} />
      <div className="px-6 py-6"><ReceiptScreen title={t('paymentSuccess')} transactionId={txnId} fields={[
        { label: t('accountNo'), value: bill.accountId },
  ...
  return (
    <div className="animate-fadeIn">
      ...
      <div className="px-6 py-6">
        <h1 className="text-[28px] font-bold text-[#212529] mb-6">{t('payWaterBill')}</h1>
        {step === 'input' && (
          <div>
            <p className="text-[18px] text-[#6C757D] mb-4">{t('accountNo')}</p>
            <div className="bg-white rounded-2xl p-6 border-2 border-[#DEE2E6] mb-6 text-center" style={{ boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}>
              <div className="text-[28px] font-bold text-[#212529] tracking-wider">{accountNo || '\u00A0'}</div>
            </div>
            <NumericKeypad value={accountNo} onChange={setAccountNo} maxLength={12} onSubmit={() => setStep('details')} submitLabel={t('search')} />
          </div>
        )}
        {step === 'details' && (
          <div>
            <div className="bg-white rounded-2xl border-2 border-[#DEE2E6] overflow-hidden mb-6" style={{ boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}>
              <div className="bg-[#00695C] text-white p-4"><h3 className="text-[20px] font-bold">Water Bill Details</h3></div>
              <div className="p-5 space-y-3">
                {[[t('accountNo'), bill.accountId], [t('name'), bill.name], [t('amountDue'), `Rs. ${bill.outstanding.toFixed(2)}`], [t('dueDate'), bill.dueDate], [t('units'), `${bill.units} KL`]].map(([l, v], i) => (
            ...
            <button onClick={() => setStep('payment')} className="w-full h-[72px] rounded-xl text-[22px] font-semibold bg-[#00695C] text-white active:opacity-90 focus:outline-2 focus:outline-[#F26522] focus:outline-offset-2" data-testid="button-pay-now">{t('payNow')} - Rs. {bill.outstanding.toFixed(2)}</button>
          </div>
        )}
        {step === 'payment' && (
          <div>
            ...
            <div className="space-y-3 mb-6">
              {['UPI', 'Card', 'Cash'].map(m => (
                <button key={m} onClick={() => setPaymentMethod(m)} className={`w-full rounded-xl p-5 text-left border-2 transition-colors focus:outline-2 focus:outline-[#006EB3] focus:outline-offset-2 ${paymentMethod === m ? 'border-[#00695C] bg-[#E0F2F1]' : 'border-[#DEE2E6] bg-white'}`} data-testid={`payment-${m}`}>
